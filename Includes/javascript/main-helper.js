$(function() {
		...
		$("textarea.input-area").keyup(function(event) {
			...
			for(var i = 0; i<split.length; i++)
			{
				...
				if(index != -1)
				{
					...
					//index translation
					var geniaal = translationAvailable(optionid, languageid, params);
			...
			{*/
				// $("div.output-area").html("<xmp>"+split+"</xmp>");
				// $("div.output-area").html(split);
	...
	function translationAvailable(optionid, languageid, array)
	{
		var theIndex = false;
			index 	 = -1;
		for (var i = 0; i < array.length; i++) {
			if (array[i].optionid == optionid) {
				if (array[i].languageid == languageid) {	
					theIndex = true;
					index = i;
					break;
				}
